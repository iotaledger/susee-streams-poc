...
async fn create_channel(channel_manager: &mut ChannelManagerPlainTextWallet) -> Result<()>{
    ...
    println!(
        "[Management Console] Announcement Link: {}\n       Tangle Index: {:#}\n",
...
async fn println_channel_status<'a> (channel_manager: &mut ChannelManagerPlainTextWallet)
{
    ...
    if let Some(author) = &channel_manager.author {
        match author.announcement_link() {
            Some(link) => {
                println!(
                    "[Management Console] A channel has been created with the following details:\n
                     Announcement Link: {}
                     Tangle Index:      {:#}\n",
                    link.to_string(), link.to_msg_index()
                );
                channel_exists = true
            },
            _ => {},
...
async fn main() -> Result<()> {
    ...
    let mut channel_manager = ChannelManager::new(
        cli.node,
        wallet,
        Some(String::from("channel-state-management-console.bin"))
    ).await;
