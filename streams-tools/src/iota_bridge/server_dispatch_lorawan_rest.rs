...
impl ServerDispatchLorawanRest for DispatchLorawanRest {
    ...
    async fn post_binary_request(self: &mut Self, dev_eui: &str, request_bytes: &[u8]) -> anyhow::Result<DispatchedRequestParts> {
        println!("[IOTA-Bridge - DispatchLorawanRest] post_binary_request() - Incoming request for dev_eui '{}' with {} bytes length", dev_eui, request_bytes.len());
        ...
        println!("[IOTA-Bridge - DispatchLorawanRest] post_binary_request() - Request is valid DispatchLorawanRest request\n{}", iota_bridge_request_parts);
