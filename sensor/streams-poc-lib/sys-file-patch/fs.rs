...
fn buffer_capacity_required(mut file: &File) -> usize {
    println!("-------------------------------- buffer_capacity_required - file.metadata().map --------------");
    let size = file.metadata().map(|m| m.len()).unwrap_or(0);
    println!("-------------------------------- buffer_capacity_required - stream_position --------------");
    let pos = file.stream_position().unwrap_or(0);
    // Don't worry about `usize` overflow because reading will fail regardless
    // in that case.
    println!("-------------------------------- buffer_capacity_required - size.saturating_sub(pos) --------------");
...
impl Read for File {
    ...
    fn read_to_end(&mut self, buf: &mut Vec<u8>) -> io::Result<usize> {
        println!("-------------------------------- read_to_end - reserve --------------");
        buf.reserve(buffer_capacity_required(self));
        println!("-------------------------------- read_to_end - default_read_to_end --------------");
