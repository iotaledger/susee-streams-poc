...
pub(crate) fn default_read_to_end<R: Read + ?Sized>(r: &mut R, buf: &mut Vec<u8>) -> Result<usize> {
    println!("-------------------------------- default_read_to_end - buf.len() --------------");
    let start_len = buf.len();
    let start_cap = buf.capacity();

    let mut initialized = 0; // Extra initialized bytes from previous loop iteration
    loop {
        println!("-------------------------------- default_read_to_end - loop START --------------");
        if buf.len() == buf.capacity() {
            println!("-------------------------------- default_read_to_end - buf.reserve(32) --------------");
            buf.reserve(32); // buf is full, need more space
        }

        println!("-------------------------------- default_read_to_end - ReadBuf::uninit --------------");
        let mut read_buf = ReadBuf::uninit(buf.spare_capacity_mut());

        // SAFETY: These bytes were initialized but not filled in the previous loop
        unsafe {
            println!("-------------------------------- default_read_to_end - read_buf.assume_init --------------");
            read_buf.assume_init(initialized);
        }

        println!("-------------------------------- default_read_to_end - match r.read_buf --------------");
        ...
        if read_buf.filled_len() == 0 {
            println!("-------------------------------- default_read_to_end - Ok(buf.len() - start_len) --------------");
            return Ok(buf.len() - start_len);
        }

        // store how much was initialized but not filled
        println!("-------------------------------- default_read_to_end - read_buf.initialized_len() --------------");
        initialized = read_buf.initialized_len() - read_buf.filled_len();
        println!("-------------------------------- default_read_to_end - read_buf.filled_len() --------------");
        let new_len = read_buf.filled_len() + buf.len();

        // SAFETY: ReadBuf's invariants mean this much memory is init
        unsafe {
            println!("-------------------------------- default_read_to_end - buf.set_len(new_len) --------------");
            buf.set_len(new_len);
        }

        println!("-------------------------------- default_read_to_end - if buf.len() == buf.capacity() --------------");
        if buf.len() == buf.capacity() && buf.capacity() == start_cap {
            ...
            loop {
                println!("-------------------------------- default_read_to_end - match r.read(&mut probe) --------------");
